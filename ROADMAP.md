...
| 48 | **Re-enable parallel wiring/phd/runtime with thread-safe output capture** | ~60 LOC | Introduced in `fe10bce` (parallelize wiring/phd/runtime: run in thread pool), reverted same day — `redirect_stdout` in `wiring.run()`/`phd.run()`/`runtime.run()` races on `sys.stdout` when called from threads: Thread A sets `sys.stdout=buf_a`, Thread B sets `sys.stdout=buf_b` (steals A's output), Thread B finishes and restores `sys.stdout=terminal` → Thread A leaks raw CHECKS output to terminal. Also causes silent crashes that drop all downstream results (lint/black/semgrep/bandit/deps/quality never print). Fix: pass an explicit `out: StringIO` parameter to each module instead of using global `redirect_stdout`, then restore the `ThreadPoolExecutor(max_workers=3)` block in runner.py. Keeps the ~2.4× speedup (26s→11s wall-clock) correctly |
