...
- Test: `python3 -m pytest tests/ -q -p no:logfire` (~610 tests across 32 files, ~38s serial)
...
- **Test run time**: thirdDraftAgentLoop full suite takes ~5 minutes (982 tests). Warn the user BEFORE starting a long run. Use `--ignore` to skip known-broken files for fast verification (~60s). audit-code suite is ~38s serial (~610 tests).
